...
public class RewardOwnerUserService : IRewardOwnerUserService
{
    ...
    public async Task<Result<IEnumerable<RewardOwnerUserProfileResponseDto>>> GetRewardOwnerUserProfilesByAuthProviderIdAsync(string authProviderId)
    {
        // 1. Validate input
        if (string.IsNullOrWhiteSpace(authProviderId))
        {
            _logger.LogWarning("GetRewardOwnerUserProfilesByAuthProviderId called with empty authProviderId");
            return Result<IEnumerable<RewardOwnerUserProfileResponseDto>>.Failure("Auth provider ID is required");
        }
        ...
        if (rewardOwnerUser == null)
        {
            _logger.LogInformation(
                ...
                authProviderId);
            // Return empty list as requested
            return Result<IEnumerable<RewardOwnerUserProfileResponseDto>>.Success(
                new List<RewardOwnerUserProfileResponseDto>());
        }
        ...
        // 4. Determine if profile exists based on RewardOwner.Name
        var rewardOwnerProfileExists = !string.IsNullOrWhiteSpace(rewardOwner.Name);

        // 5. Map and build response
        var rewardOwnerDto = _mapper.Map<RewardOwnerDto>(rewardOwner);
        var rewardOwnerUserDto = _mapper.Map<RewardOwnerUserDto>(rewardOwnerUser);

        var profile = new RewardOwnerUserProfileResponseDto
        {
            RewardOwnerProfileExists = rewardOwnerProfileExists,
            RewardOwner = rewardOwnerDto,
            RewardOwnerUser = rewardOwnerUserDto
        };

        _logger.LogInformation(
            "RewardOwnerUserProfile found for authProviderId: {AuthProviderId}, RewardOwnerId: {RewardOwnerId}, UserId: {UserId}, ProfileExists: {ProfileExists}",
            authProviderId, rewardOwner.Id, rewardOwnerUser.Id, rewardOwnerProfileExists);

        return Result<IEnumerable<RewardOwnerUserProfileResponseDto>>.Success(
            new List<RewardOwnerUserProfileResponseDto> { profile });
    }
}
